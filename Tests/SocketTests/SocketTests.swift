...
struct SocketTests {
    ...
    @Test("TCP NoDelay Option Configuration")
    ...
    @Test("TCP NoDelay Behavior with Connected Sockets")
    func testTCPNoDelayBehavior() async throws {
        ...
        let port = UInt16.random(in: 8080 ..< .max)
        Self.logger.info("Testing TCP_NODELAY behavior on port \(port)")
        let address = IPv4SocketAddress(address: .any, port: port)
        ...
        Self.logger.info("Successfully tested TCP_NODELAY option setting and data transmission")
    }
